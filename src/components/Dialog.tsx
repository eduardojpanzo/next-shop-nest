...
export function Dialog({ trigger, title, desc, children }: DialogProps) {
  return (
    <D.Root>
      ...
      <D.Portal>
        <D.Overlay className="bg-blackA9 data-[state=open]:animate-overlayShow fixed inset-0" />
        <D.Content className="data-[state=open]:animate-contentShow fixed top-[50%] left-[50%] max-h-[85vh] w-[90vw] max-w-[450px] translate-x-[-50%] translate-y-[-50%] rounded-[6px] bg-white p-[25px] shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px] focus:outline-none">
          <D.Title className="text-mauve12 m-0 text-[17px] font-medium">
            {title}
          </D.Title>
          <D.Description className="text-mauve11 mt-[10px] mb-5 text-[15px] leading-normal">
            {desc}
          </D.Description>
          {children}
          <D.Close asChild>
